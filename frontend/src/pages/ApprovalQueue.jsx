...
export default function ApprovalQueue() {
  ...
  // ---- AI Suggestion (now sends only transaction_id) ----
  const fetchAiSuggestion = async (tx) => {
    setAiLoading(true);
    try {
      // ✅ Send only the transaction_id
      const res = await api.post('/agent/suggest', { transaction_id: tx.transaction_id });
  ...
  const acceptAiSuggestion = () => {
    if (!aiSuggestion || !selectedTx) return;
    setNewDecision(aiSuggestion.suggested_decision);
    setReason(aiSuggestion.reason);
  ...
  const exportCSV = () => {
    ...
    const rows = auditLog.map(log => [
      ...
      log.amount || 0,
      log.probability ? `${(log.probability * 100).toFixed(2)}%` : '—',
              ...
              <tbody>
                {pendingReviews.map(tx => (
                  <tr key={tx.transaction_id || tx.id}>
                    ...
                    {/* FIXED: Changed .toFixed(1) to .toFixed(2) */}
                    <td>{((tx.probability || 0) * 100).toFixed(2)}%</td>
              ...
              {auditLog.length === 0 ? (
                ...
                auditLog.slice(0, 100).map((log, idx) => (
                  <tr key={log.id || idx}>
                    ...
                    {/* FIXED: Changed .toFixed(1) to .toFixed(2) */}
                    <td>{log.probability ? `${(log.probability * 100).toFixed(2)}%` : '—'}</td>
      ...
      {showAiModal && aiSuggestion && (
        <div className="modal-overlay open" onClick={() => setShowAiModal(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p><strong>Suggested Decision:</strong> <span className={`badge decision ${aiSuggestion.suggested_decision}`}>{aiSuggestion.suggested_decision}</span></p>
              {/* FIXED: Changed .toFixed(1) to .toFixed(2) to match the 0.06% reasoning text */}
              <p><strong>Probability:</strong> {(aiSuggestion.probability * 100).toFixed(2)}%</p>
              <p><strong>Confidence:</strong> {aiSuggestion.confidence}</p>
              <p><strong>Reason:</strong> {aiSuggestion.reason}</p>
              {aiSuggestion.explanation && Object.keys(aiSuggestion.explanation).length > 0 && (
                <details>
                  <summary>Feature Impact (SHAP)</summary>
                  <ul style={{ paddingLeft: '1.2rem' }}>
                    {Object.entries(aiSuggestion.explanation)
                      .slice(0, 5)
                      .map(([feature, impact]) => (
                        <li key={feature}>
                          {feature}: <span style={{ color: impact > 0 ? '#ef4444' : '#10b981' }}>
              ...
              <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
                <button className="btn-primary" onClick={acceptAiSuggestion}>
                  ✅ Accept & Override
                </button>
