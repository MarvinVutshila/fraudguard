...
from fraud_detection.application.services.email_service import send_email
...
async def approve_user(approval: UserApprove, background_tasks: BackgroundTasks, current_user=Depends(get_current_admin)):
    try:
        ...
        if approval.approve:
            subject = "FraudGuard Account Approved"
            body = f"""Dear {user_email},

Your FraudGuard account has been approved by an administrator. You can now log in to the dashboard.
...
"""
            ...
            subject = "FraudGuard Account Rejected"
            body = f"""Dear {user_email},

Your registration request for FraudGuard has been rejected by an administrator.
...
"""
        ...
        logger.info(f"✅ User {user['username']} set to {new_status}, email sent")
...
async def block_user(user_id: int, block_data: BlockRequest, background_tasks: BackgroundTasks, current_user=Depends(get_current_admin)):
    ...
    if user['role'] == 'admin':
        raise HTTPException(403, "Admin users cannot be blocked for security reasons.")
    ...
    subject = "FraudGuard Account Blocked"

    # Use the admin's reason or a default
    reason_text = block_data.reason if block_data.reason else "Account blocked due to security concerns."

    body = f"""Dear {user_email},

Your FraudGuard account has been blocked by an administrator.

Reason: {reason_text}

Our security team will investigate this matter further. If you believe this is an error, please contact support immediately.
...
"""
    ...
    logger.info(f"✅ User {user['username']} blocked and all sessions invalidated, email sent")
    return {"message": f"User {user_id} has been blocked and logged out"}

# ---------- Unblock user (ADMIN ONLY) ----------
@router.post("/users/{user_id}/unblock")
async def unblock_user(user_id: int, background_tasks: BackgroundTasks, current_user=Depends(get_current_admin)):
    ...
    # ---------- Send email notification ----------
    user_email = user['username']
    subject = "FraudGuard Account Unblocked"
    body = f"""Dear {user_email},

Your FraudGuard account has been unblocked. You can now log in again.
...
"""
    ...
    logger.info(f"✅ User {user['username']} unblocked, email sent")
    return {"message": f"User {user_id} has been unblocked"}

# ---------- Delete user (ADMIN ONLY) - COMPLETE CASCADE DELETE ----------
@router.delete("/users/{user_id}")
async def delete_user(user_id: int, background_tasks: BackgroundTasks, current_user=Depends(get_current_admin)):
    """
    ...
    # ---------- Send email notification ----------
    user_email = username
    subject = "FraudGuard Account Deleted"
    body = f"""Dear {user_email},

Your FraudGuard account has been permanently deleted by an administrator.
...
# ════════════════════════════════════════════════════════════════
# 🔔 ALIAS ROUTE – FRONTEND EXPECTS /admin/alerts (without "security/")
# ════════════════════════════════════════════════════════════════
@router.get("/alerts")
async def get_alerts_alias(current_user=Depends(get_current_admin)):
    """Alias for /security/alerts – used by the frontend."""
    return await get_security_alerts(current_user)
